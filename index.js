...
wss.on('connection', function(clientWebsocketRaw, req) {
  ...
  let clientWebsocket = new WebSocketClient().initializeFromRaw(
    clientWebsocketRaw,
    'api2client',
  ...
  clientWebsocket
    .waitForMessage({
      condition: obj =>
        obj.from == 'client' &&
        obj.type == 'call' &&
        obj.command == 'api-connectBackend',
      keepWhenHit: true
    })
    .then(clientCallRequest => {
      if (backendWebsocket.isOpen) return;
      new BootstrapStep({
        websocket: backendWebsocket,
        actor: websocket => {
          websocket.initialize(backendInfo.url, 'api2backend', {
            func: WebSocket,
  ...
  clientWebsocket
    .waitForMessage({
      condition: obj =>
        obj.from == 'client' &&
        obj.type == 'call' &&
        obj.command == 'backend-connectWhatsApp',
      keepWhenHit: true
    })
    .then(clientCallRequest => {
      ...
      new BootstrapStep({
        ...
        request: {
          ...
          successCondition: obj =>
            obj.type == 'resource_connected' &&
            obj.resource == 'whatsapp' &&
            obj.resource_instance_id
        }
      })
        .run(backendInfo.timeout)
        .then(backendResponse => {
          backendWebsocket.activeWhatsAppInstanceId =
            backendResponse.data.resource_instance_id;
          backendWebsocket
            .waitForMessage({
              condition: obj =>
                obj.type == 'resource_gone' && obj.resource == 'whatsapp',
              keepWhenHit: false
            })
            .then(() => {
  ...
  clientWebsocket
    .waitForMessage({
      condition: obj =>
        obj.from == 'client' &&
        obj.type == 'call' &&
        obj.command == 'backend-disconnectWhatsApp',
      keepWhenHit: true
    })
    .then(clientCallRequest => {
      if (!backendWebsocket.isOpen) {
      ...
      new BootstrapStep({
        ...
        request: {
          ...
          successCondition: obj =>
            obj.type == 'resource_disconnected' &&
            obj.resource == 'whatsapp' &&
            obj.resource_instance_id ==
              backendWebsocket.activeWhatsAppInstanceId
        }
  ...
  clientWebsocket
    .waitForMessage({
      condition: obj =>
        obj.from == 'client' &&
        obj.type == 'call' &&
        obj.command == 'backend-generateQRCode',
      keepWhenHit: true
    })
    .then(clientCallRequest => {
      if (!backendWebsocket.isOpen) {
      ...
      new BootstrapStep({
        ...
        .then(backendResponse => {
          ...
          backendWebsocket
            .waitForMessage({
              condition: obj =>
                obj.type == 'whatsapp_message_received' &&
                obj.message &&
                obj.message_type &&
                obj.timestamp &&
                obj.resource_instance_id ==
